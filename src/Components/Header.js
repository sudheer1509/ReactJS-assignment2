import "./Header.css";
import { useState } from "react";

function Header() {
  return (
    <>
      <div className="topbar">
        <div>
          <div className="logo"> EDYODA </div>
          <div className="logoStories">Stories</div>
        </div>
        <div className="exploreCategories">
          <span>Explore catagories</span>
          <img
            src="data:image/png;base64,"
            alt="icon"
            id="icon"
            width="15px"
          />
        </div>
        <div className="headerText">
          <div>EdYoda is free learning and knowledge </div>
          <div>sharing platform for techies</div>
        </div>
        <a href="https://www.edyoda.com/home" target="_blank">
          <button className="buttonToMainWebsite">Go To main Website</button>
        </a>
      </div>
      <TrendingSection />
    </>
  );
}

function TrendingSection() {
  const Categories = [
    "All",
    "Artificial Intelligence",
    "Cloud Computing",
    "DevOps",
    "Programming Languages",
    "Mobile Application Development",
    "Technology and Tools",
    "Get a Job in a Tech Company",
    "Others",
  ];
  const [category, setCategory] = useState(Categories[0]);

  return (
    <>
      <div className="container">
        <div id="catagories" style={{ paddingTop: "120px" }}>
          <div style={{ paddingBottom: "10px" }}>
            <h1 className="heading">Trending Posts</h1>
            <img
              src="https://cdn4.iconfinder.com/data/icons/fedora-color/512/filter-512.png"
              alt=""
              width="20px"
            />
            <span className="filter">Filter by Catagory</span>
            <br />
            {Categories.map((item, index) => (
              <button
                className="button"
                onClick={() => setCategory(item)}
                style={{
                  color: category === item ? "#fff" : "#0084ff",
                  backgroundColor: category === item ? "#0084ff" : "white",
                  border: category === item ? "none" : "1px solid #0084ff",
                }}
              >
                {item}
              </button>
            ))}
          </div>
          <br></br>
        </div>
      </div>
    </>
  );
}
export default Header;
